...
namespace SetUserEnvVars
{
    public class RegistryHelper
    {
        ...
        /// <returns></returns>
        public static RegistryKey Open(RegistryKey baseKey, string registryPath, RegistryKeyPermissionCheck permissionCheck = RegistryKeyPermissionCheck.ReadWriteSubTree)
        {
            var key = baseKey;
            var pathParts = new Queue<string>(registryPath.Split('\\'));
            while (pathParts.TryDequeue(out string keyName))
            {
                key = baseKey.OpenSubKey(keyName, permissionCheck);
                if (key == null)
                    throw new InvalidRegistryPathException($@"Invalid registry key: sub key ""{keyName}"" does not exist.");
            }
        ...
        public static object SetRegistryValue(RegistryKey root, string registryPath, string registryValueName, object value)
        {
            object initialValue = null;

            try
            {
                var registryPathKey = RegistryHelper.Open(root, registryPath, RegistryKeyPermissionCheck.ReadWriteSubTree);
                try
                {
                ...
                throw new Exception("Unable to " + (initialValue == null ? "set new" : "overwrite") + $@" registry value (""{registryValueName}"").", e);
            }
        ...
        public static void UnsetRegistryValue(RegistryKey root, string registryPath, string registryValueName)
        {
            try
            {
                var registryKey = RegistryHelper.Open(root, registryPath, RegistryKeyPermissionCheck.ReadWriteSubTree);
